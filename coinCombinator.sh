...
findMaximum () {
	...
	for value in $@
	do
		array[$index]=`echo $value | awk -F. '{print $1}'`
		(( index++ ))
	done
        for ((counter1=0;counter1<=${#array[@]}-1;counter1++))
        do
                for((counter2=0;counter2<${#array[@]}-counter1-1;counter2++))
                do
                        if [ ${array[counter2]} -lt ${array[$((counter2+1))]} ] 
                        then
                                temp=${array[counter2]}
	...
	for i in ${!toss[@]}
	do
		dictionaryVal=`echo ${toss[$i]} | awk -F. '{print $1}'`
		if [ $dictionaryVal -eq $max ]
		then
			echo -n "Winning Pair is " $i
			echo "with Percent of " $max
		else
			continue
		fi
	done
